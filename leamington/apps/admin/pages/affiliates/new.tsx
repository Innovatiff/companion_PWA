...
export default function NewAffiliate({ viewer, values, error }: Props) {
  ...
  const err = (key: string, ...keys: string[]) =>
    error && [key, ...keys].includes(error) ? <p className="note bad hint" id={`${key}-err`}>{n.errors[error]}</p> : null;
  return (
    <Page viewer={viewer} section="affiliates" title={n.title} subtitle={n.subtitle}>
      <Card className="form">
        <form method="post" action="/api/affiliates/create">
          {error && !["name", "commission", "login", "login_taken"].includes(error) && <Note kind="bad">{n.errors.invalid}</Note>}
