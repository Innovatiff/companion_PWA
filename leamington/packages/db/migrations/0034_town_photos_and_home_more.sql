-- 0034_town_photos_and_home_more.sql
-- Real photos of clients' hometowns, and more real sections on Hoy's home.
--
--   * municipality_photos: one photo per town, from Wikimedia Commons, fetched
--     server-side (services/ingest/scripts/fetch-town-photos.mjs) and served by
--     Hoy from our own domain. Only freely licensed photos, each with its author,
--     license and source page, which Hoy shows as a credit line. The image comes
--     from the town's own Wikipedia article, whose coordinates must be within
--     25 km of the town, so a same-name place elsewhere is never used.
--   * app.home_more: the extra home sections, built from existing real data.
--     Each is null when there is nothing real to show, with its own validity.

create table municipality_photos (
  municipality_id bigint primary key references municipalities(id) on delete cascade,
  ...
  source_page_url text not null,          -- the file's page: author and license live there
  article_url     text not null,          -- the town's Wikipedia article the photo came from
...
alter table municipality_photos enable row level security;
create policy municipality_photos_read on municipality_photos for select using (auth.role() in ('authenticated', 'service_role'));

create or replace function app.town_photo(p_municipality_id bigint)
returns jsonb language sql stable set search_path = public, app as $$
  select jsonb_build_object('municipality_id', p.municipality_id, 'author', p.author, 'license', p.license,
                            'license_url', p.license_url, 'source_page_url', p.source_page_url,
...
begin
  ...
  return jsonb_build_object(
    ...
    -- Today's forecast for the towns they watch (not their home, which has its own line).
    'watch_weather', (
       select jsonb_agg(jsonb_build_object('id', (t->>'id')::bigint, 'name', t->>'name',
        ...
        where not (t->>'is_home')::boolean
          ...
          and (t->'days'->0->>'date')::date = v_today),
    ...
    'alerts', case when v_w->>'alerts_state' in ('current', 'stale') then jsonb_build_object(
       ...
       'valid_until', case when v_w->>'alerts_checked_at' is not null
                           then (v_w->>'alerts_checked_at')::timestamptz + interval '45 minutes' end,
       'agency', v_w->>'agency',
