-- 0035_stale_warnings_stay_visible.sql
-- A stale warnings check stays on screen.
--
-- home_more (0034) gave the warnings card a validity of checked_at + 45 minutes.
-- For a stale check that moment has already passed, so the page's offline
-- script would remove "No hemos podido revisar los avisos..." as it loaded, and
-- the person would see nothing: silence read as calm. A stale card is now valid
-- for 45 minutes from the render; a current one still expires 45 minutes after
-- the check, when it would itself become stale.
